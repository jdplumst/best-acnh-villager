...
export const villagerRouter = createTRPCRouter({
  ...
  getStatistics: publicProcedure.query(async ({ ctx }) => {
    ...
    const speciesStats: Record<string, Stats> = {};
    villagers.forEach((v) => {
    ...
    for (const s in speciesStats) {
      speciesData[s] = speciesStats[s]!.ratingSum / speciesStats[s]!.count;
    }

    const genderStats: Record<string, Stats> = {};
    villagers.forEach((v) => {
    ...
    const personalityStats: Record<string, Stats> = {};
    villagers.forEach((v) => {
    ...
    const subtypeData: Record<string, number> = {};
    for (const s in subtypeStats) {
      subtypeData[s] = subtypeStats[s]!.ratingSum / subtypeStats[s]!.count;
    }
